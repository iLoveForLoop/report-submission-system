...
function SectionHeader({
    ...
    return (
        <div className={`flex items-center justify-between border-b-2 border-border px-5 py-3 ${headerColor ?? 'bg-muted/40'}`}>
            <div className="flex items-center gap-2.5">
                <span className={`text-sm font-semibold tracking-tight ${hasColor ? 'text-white' : 'text-foreground'}`}>
                    {title}
                </span>
                {badge !== undefined && (
                    <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${hasColor ? badgeClasses[badgeVariant] : defaultBadgeClasses[badgeVariant]}`}>
                        {badge}
                    </span>
                )}
            </div>
            ...
            <div className="flex-1 space-y-6 bg-background p-6 md:p-8">
                ...
                <div className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-0.5 text-sm text-muted-foreground">
                            Review and manage officer report submissions
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            ...
                            Review Queue
                            {pending_count > 0 && (
                                <span className="rounded bg-white/20 px-1.5 py-0.5 text-xs font-bold leading-none">
                                    {pending_count}
                                </span>
                            )}
                        </Link>
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    {stats.map((stat, i) => (
                        <Link
                            key={i}
                            href={stat.link}
                            className={`group flex items-center gap-4 rounded border-2 border-border bg-card p-4 transition-colors hover:bg-muted/50 border-l-4 ${stat.accent}`}
                        >
                            <div className="min-w-0 flex-1">
                                <p className="text-xs font-medium tracking-wide text-muted-foreground uppercase">
                                    {stat.title}
                                </p>
                                <p className={`mt-1 text-3xl font-bold leading-none ${stat.valueColor}`}>
                                    {stat.value}
                                </p>
                ...
                <div className="grid gap-5 lg:grid-cols-3">
                    ...
                    <div className="space-y-5 lg:col-span-2">
                        ...
                        <div className="rounded border-2 border-border bg-card">
                            <SectionHeader
                                ...
                                action={{ label: 'View all', href: '/focal-person/submissions?filter=pending' }}
                            />
                            ...
                            {pending_submissions.length === 0 ? (
                                <div className="flex flex-col items-center justify-center gap-2 py-14 text-center">
                                    ...
                                    <p className="text-xs text-muted-foreground/60">No submissions pending review.</p>
                                </div>
                            ) : (
                                <div className="divide-y-2 divide-border">
                                    {pending_submissions.map((sub) => (
                                        <div key={sub.id} className="px-5 py-4 transition-colors hover:bg-muted/30">
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="flex min-w-0 items-start gap-2">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="mt-0.5 text-xs text-muted-foreground">{sub.program}</p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1.5">
                                                <MetaItem icon={Users}    label="Officer"   value={sub.officer} />
                                                <MetaItem icon={MapPin}   label="Cluster"   value={sub.cluster} />
                                                <MetaItem icon={Clock}    label="Submitted" value={formatDateTime(sub.submitted_at)} />
                                                {sub.deadline && (
                                                    <MetaItem icon={Calendar} label="Deadline" value={formatDate(sub.deadline)} />
                                                )}
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <div className="rounded border-2 border-red-400 bg-card dark:border-red-700">
                                ...
                                <div className="divide-y-2 divide-red-200 dark:divide-red-900/40">
                                    {overdue_reports.map((report) => (
                                        <div key={report.id} className="px-5 py-4 transition-colors hover:bg-red-50/40 dark:hover:bg-red-950/10">
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="flex min-w-0 items-start gap-2">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="mt-0.5 text-xs text-muted-foreground">{report.program}</p>
                                                    </div>
                                                </div>
                                                <span className="shrink-0 rounded border border-red-300 bg-red-100 px-2 py-0.5 text-xs font-semibold text-red-700 dark:border-red-700 dark:bg-red-900/40 dark:text-red-400">
                                                    {report.days_overdue}d overdue
                                                </span>
                                            </div>

                                            <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2">
                                                <MetaItem icon={Calendar} label="Deadline" value={formatDate(report.deadline)} />
                                                <MetaItem
                                                    icon={Users}
                                                    label="Submitted"
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-xs text-muted-foreground">
                                                        {report.total_officers > 0
                                                            ? Math.round((report.submitted / report.total_officers) * 100)
                                                            : 0}%
                                                    </span>
                                                </div>
                                            </div>
                    ...
                    <div className="space-y-5">
                        ...
                        {/* Recent Activity */}
                        <div className="rounded border-2 border-border bg-card">
                            <SectionHeader title="Recent Activity" headerColor="bg-slate-600" />
                            ...
                            {recent_activity.length === 0 ? (
                                ...
                                <div className="divide-y-2 divide-border">
                                    {recent_activity.map((activity) => {
                                        const isApproved = activity.type === 'accepted';
                                        return (
                                            <div key={activity.id} className="flex items-start gap-3 px-4 py-3.5 transition-colors hover:bg-muted/30">
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    <p className="mt-1 text-xs text-muted-foreground/60">
                                                        {formatDateTime(activity.actioned_at)}
                                                    </p>
                                                </div>

                                                <span className={`shrink-0 self-start rounded px-1.5 py-0.5 text-xs font-medium ${
                                                    isApproved
                                    ...
                                    {assigned_programs.map((program) => (
                                        ...
                                        >
                                            <div className="min-w-0">
                                                ...
                                                <p className="mt-0.5 text-xs text-muted-foreground">
                                                    {program.total_reports} total reports
                                                </p>
                                            </div>
